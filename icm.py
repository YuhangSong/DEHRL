...
def create_commands(session, num_workers, remotes, env_id, logdir, shell='bash',
    ...
    # --exp two_sequence --obs-type 'image' --reward-level 2 --log-behavior-interval 5 --aux r_0
    base_cmd += ['--exp', args.exp]
    base_cmd += ['--obs-type', args.obs_type]
    base_cmd += ['--reward-level', args.reward_level]
    base_cmd += ['--log-behavior-interval', args.log_behavior_interval]
    base_cmd += ['--aux', args.aux]
